...
const UserManagement = () => {
  ...
  const fetchUsers = useCallback(async () => {
    const response = await fetch(
      `${baseUrl}/user/allusers?search=${searchTerm}&status=${tabState === 'active' ? 'Y' : 'N'}&page=${currentPageState[tabState]}&pageSize=${pageSizeState[tabState]}`
    )
        ...
        <div className={styles.tableContainer}>
          ...
          >
            <Tab eventKey="active" title="Active">
              <div>
                {Object.keys(activeUsersState).length === 0 ||
                      ...
                      {activeUsersState.results.map((user, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>{user.role}</td>
                            <td>
                              {user.active === 'Y' ? 'active' : 'suspend'}
                            </td>
                            <td>
                              <EditUserModal
                                updateUserList={fetchUsers}
                                username={user.nci_username}
                              />
                              <a
                                className={styles.tableLink}
                                href="/#"
                                aria-label="suspend"
                                onClick={(e) => handleStatusUpdated(e, user)}
                              >
                                <TimesIcon />
                              </a>
                            </td>
            ...
            <Tab eventKey="suspend" title="Suspend">
              <div>
                {Object.keys(suspendUsersState).length === 0 ||
