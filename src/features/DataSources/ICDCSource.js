...
import { Button, Table, InputGroup, FormControl, Tabs, Tab} from 'react-bootstrap';
...
const ICDCSource = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`${baseUrl}/datamodel/search?model=ICDC&page=1&pageSize=10`);
  ...
  const selectTabHandle = async (type) => {
    setTabState(type);
    const response = await fetch(`${baseUrl}/datamodel/search?keyword=${searchTerm}&model=ICDC&type=${type}&page=${currentPageState[type]}&pageSize=${pageSizeState[type]}`);
    const data = await response.json();
  ...
  const handlePageClick = async (x) => {
    ...
    const response = await fetch(`${baseUrl}/datamodel/search?keyword=${searchTerm}&model=ICDC&type=${tabState}&page=${page}&pageSize=${pageSizeState[tabState]}`);
    const data = await response.json();
  ...
  const handleSearchTrigger = async (keyword) => {
    const response = await fetch(`${baseUrl}/datamodel/search?keyword=${searchTerm}&model=ICDC&type=${tabState}&page=page=${currentPageState[tabState]}&pageSize=${pageSizeState[tabState]}`);
    const data = await response.json();
    switchSetResponse(tabState, data);
  }
  ...
  return (
    <DashboardContainer>
      <SectionContainer>
        <SectionHeader>
          <SectionTitle>Integrated Canine Data Commons</SectionTitle>
          <div>
